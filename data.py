...
def generate_terminal_call_data(config):
...
def generate_boundary_call_data(config):
    ...
    # C_max = black_scholes_call_solution(S_max, config["K"], config["T"] - t_vals, config["r"], config["sigma"])
...
def generate_terminal_put_data(config):
    S = np.random.uniform(config["min_S"], config["max_S"], (config["N_data"], 1))
    t = np.full_like(S, config["T"])  
    C = np.maximum(config["K"] - S, 0.0)
    C += np.random.normal(config["bias"], config["noise_variance"], size=C.shape)
...
def generate_boundary_put_data(config):
    S_min = np.full((config["N_data"], 1), config["min_S"], dtype=np.float64)
    S_max = np.full((config["N_data"], 1), config["max_S"], dtype=np.float64)
    t_vals = np.random.uniform(0, config["T"], (config["N_data"], 1))
    # Boundary values
    C_min = -1 * np.ones_like(S_min)
    C_max = np.zeros_like(S_max)  # C(max_S, t) = 0
    C_max += np.random.normal(config["bias"], config["noise_variance"], size=C_max.shape)
